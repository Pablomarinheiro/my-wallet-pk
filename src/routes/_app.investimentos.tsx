  ...
  head: () => ({
    meta: [
      { title: "Investimentos e patrimônio — My Wallet" },
      {
        name: "description",
        content:
          "Acompanhe aportes, resgates, total investido, valor atual e rentabilidade separados do saldo em conta.",
      },
      { property: "og:title", content: "Investimentos e patrimônio — My Wallet" },
      {
        property: "og:description",
        content: "Patrimônio aplicado separado do dinheiro disponível em conta.",
      },
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="rounded-3xl sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {kind === "deposit" ? "Novo aporte" : "Novo resgate"} — {investment.name}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card className="rounded-3xl border-border/70 shadow-soft">
          <CardContent className="p-5">
            ...
            <div
              className={`mt-1 text-2xl font-bold ${totals.profit >= 0 ? "text-success" : "text-destructive"}`}
            >
        ...
        <Card className="rounded-3xl border-dashed">
          <CardContent className="p-10 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Cadastre seus ativos para separar patrimônio aplicado do saldo em conta.
            </p>
            <div className="mt-4">
              <InvestmentDialog
                trigger={
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {Number(i.quantity)} un · preço médio {currency(Number(i.avg_price))} ·{" "}
                      {byInvestment.get(i.id) ?? 0} movimentação(ões)
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <div
                      className={`text-[11px] ${profit >= 0 ? "text-success" : "text-destructive"}`}
                    >
                      {profit >= 0 ? "+" : ""}
                      {currency(profit)} · {pct.toFixed(2)}%
                    </div>
                  </div>
                  <div className="flex gap-1">
                    <MovementDialog
                      ...
                      kind="deposit"
                      trigger={
                        <Button variant="ghost" size="icon" aria-label="Aportar">
                    ...
                    <MovementDialog
                      ...
                      kind="withdraw"
                      trigger={
                        <Button variant="ghost" size="icon" aria-label="Resgatar">
                        ...
                        if (confirm(`Excluir "${i.name}"?`))
                          del.mutate(i.id, {
                            onError: (e: { message?: string }) =>
                              toast.error(e?.message ?? "Erro ao excluir investimento"),
                          });
                      }}
                    >
