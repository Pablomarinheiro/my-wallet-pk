...
function TransferPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      toast.error(getErrorMessage(e, "Falha ao transferir"));
    }
  }
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr]">
        <Card className="rounded-3xl border-border/70 shadow-soft">
          <CardContent className="space-y-4 p-6">
            ...
            >
              {upsert.isPending && <Loader2 className="h-4 w-4 animate-spin" />} Confirmar
              transferência
            </Button>
          </CardContent>
        </Card>
                      ...
                      onClick={() =>
                        del.mutate(h.id, {
                          onError: (e: { message?: string }) =>
                            toast.error(e?.message ?? "Erro ao excluir transferência"),
                        })
                      }
