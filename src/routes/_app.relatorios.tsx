    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="relatorios" className="space-y-6">
        <TabsList className="rounded-2xl">
          ...
          <TabsTrigger value="importar" className="rounded-xl">
            Importar CSV
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Card className="rounded-3xl border-border/70 shadow-soft">
              <CardContent className="p-5">
                ...
                <div
                  className={`mt-1 text-2xl font-bold ${totals.balance >= 0 ? "text-primary" : "text-destructive"}`}
                >
              ...
              {[
                {
                  icon: FileType2,
                  label: "PDF",
                  desc: "Relatório visual pronto para imprimir",
                  kind: "pdf" as const,
                },
                {
                  icon: FileSpreadsheet,
                  label: "Excel",
                  desc: "Planilha .xlsx com todos os dados",
                  kind: "excel" as const,
                },
                {
                  icon: FileText,
                  label: "CSV",
                  desc: "Dados brutos separados por ponto e vírgula",
                  kind: "csv" as const,
                },
              ].map((e) => (
                <button
                  key={e.label}
                  onClick={() => doExport(e.kind)}
                ...
                >
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground">
          ...
          <Card className="rounded-3xl border-border/70 shadow-soft">
            <CardHeader>
              <CardTitle className="text-base">
                Prévia — {filtered.length} transação(ões) · {period}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                    ...
                    <TableBody>
                      {filtered.slice(0, 20).map((t) => (
                        <TableRow key={t.id}>
                          ...
                          <TableCell
                            className={`text-right font-bold ${t.type === "income" ? "text-success" : ""}`}
                          >
